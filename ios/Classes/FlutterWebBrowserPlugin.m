        ...
        if (@available(iOS 9.0, *)) {
            SFSafariViewController *sfvc = [[SFSafariViewController alloc] initWithURL:URL];
                    ...
                    if(controlColorArg != (id)[NSNull null]) {
                        if (@available(iOS 10.0, *)) {
                            UIColor *color = [FlutterWebBrowserPlugin colorFromHexString:controlColorArg];
                            sfvc.preferredControlTintColor = color;
                            
                            sfvc.navigationController.navigationBar.titleTextAttributes
